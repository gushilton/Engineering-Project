          ...
          if (mysqli_query($dbCon,$query)){
            if(mysqli_query($dbCon,"GRANT ALL PRIVILEGES on *.* to '".$header->{'user'}."'@'localhost' WITH GRANT OPTION")){
                if(mysqli_query($dbCon,"FLUSH PRIVILEGES")){
                  $addquery = "
                    INSERT INTO personnel.OnBoard
                    (CompanyID,RoleID,UserName,FirstName,LastName,EmbarkedDate,DisembarkDate)
                    VALUES
                    (".(int)$header->{'empID'}.",".(int)$header->{'roleID'}.",'".$header->{'user'}."','".$header->{'fName'}."','".$header->{'lName'}."','".$header->{'user'}."','".$header->{'embDate'}."','".$header->{'disDate'}."')
                  ";
                  if (mysqli_query($dbCon,$addquery)){
                    echo "person added";
                  }
                                    $addHisquery = "
                                      INSERT INTO personnel.OnAndHistory
                                      (CompanyID,UserName,FirstName,LastName)
                                      VALUES
                                      (".(int)$header->{'empID'}.",'".$header->{'user'}."','".$header->{'fName'}."','".$header->{'lName'}."')";
            ...
            echo "user not added : ".$mysqli_error($dbCon). " 	: ".$query;
          }
        }
    }
?>
