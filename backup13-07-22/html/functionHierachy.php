     ...
     if(!$dbCon){
        ...
        while($row = mysqli_fetch_assoc($result)){
          ...
          while($rowDiv2 = mysqli_fetch_assoc($resDiv2)){
            $queryDiv3 = "SELECT * FROM Operations.Function_Hierachy WHERE Division3 != '' AND Division2 = '".$rowDiv2['Division2']."' AND Division1 = '".$rowDiv2['Division1']."'";
            $resDiv3 = mysqli_query($dbCon,$queryDiv3);
            $division3 = array();
            while($rowDiv3 = mysqli_fetch_assoc($resDiv3)){
                array_push($division3,$rowDiv3);
            }
