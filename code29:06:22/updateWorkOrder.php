 <?php
     if ($_SERVER['REQUEST_METHOD'] == "POST"){
         ...
         if (!$dbCon){
             ...
             $rInsert = "";
             $rUpdate = "";
             $updateQuery = "UPDATE Operations.MaintenanceTasks SET Date_Complete = '".$header->{'Date'}."',Complete = 1,completeBy = ANY(SELECT CompanyID FROM personnel.OnBoard WHERE UserName = '".$username."' )  WHERE yearID = ".$header->{'workOrder'}->{'yearID'}." AND JobID = ".$header->{'workOrder'}->{'JobID'};
             ...
             $response = array("status"=>"success","update"=>$rUpdate,"INSERT"=>$rInsert);
